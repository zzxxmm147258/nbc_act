...
var TitleObject = {
    ...
    save : function(){
        ...
        if(this.isEdit){
            $('#editContent').children().eq(this.index).html('<div class="item" flog="title"><h1 class="titleObject '+ classIndex+'" style="'+ style +'"><label>'+ title +'</label></h1></div>');
        }else{
            $('#editContent').append('<div class="item" flog="title"><h1 class="titleObject '+ classIndex+'" style="'+ style +'"><label>'+ title +'</label></h1></div>');
        }
        this.isEdit = false;
        layer.close(pageii);
    }
...
var Path = {
    ...
    save : '/main/case/editor',
    selectById : '/main/case/selectById.ajax'
};
    ...
    grade : function(){
        ...
        if(!!data){
            for(var i = 0; i < data.length; i++){
                var s = i == 0 ? 'selected' : '';
                str += '<option name="gradeId" value="'+ data[i].code +'" index="'+ i +'" '+ s +'>'+ data[i].cname +'</option>';
            }
        }
        $('#gradeId').html(str);
        var o = $('#gradeId');
...
var  FormSave = function(){
    var gradeId = $('#gradeId').val(),
      ...
      switch(true){
        case $.trim(name).length === 0:
            ...
            alert('项目名称不能为空');
            break;
        case $.trim(school).length === 0:
            isSave = false;
            alert('学校名称不能为空');
            break;
        case $.trim(date_time).length === 0:
            isSave = false;
            alert('时间不能为空');
            break;
    ...
    $('#detail').val($('#editContent').html());

    if(isSave){
        $('#mainForm').submit();
    //    var isRecommend = $('#school').val(),
    //        detail = encodeURIComponent(encodeURIComponent($('#editContent').html()));
    //    var param = "typeId="+typeId+"&name="+name+"&des="+encodeURIComponent(encodeURIComponent(des))+"&gradeId="+gradeId.join(',')+"&classId="+classId.join(',')+"&isRecommend="+isRecommend+"&detail="+detail+"&imgUrl="+imgUrl;
    //console.log(param);
    //    $.ajax({
...
var updataInfo = {
    ...
    layoutPage : function(){
        ...
        $('#mId').val(self.data.mId);
        $('#school').val(self.data.school);
        $('#date_time').val(self.data.date_time);
        $('#updataImg').attr('src',Path.base + '/image/'+self.data.imgUrl+"/cut001");
        $('#des').val(self.data.des);
        $('#editContent').html(self.data.detail);
    }
};
