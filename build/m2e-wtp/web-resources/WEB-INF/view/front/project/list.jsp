    ...
    <div class="container clear" >
      <div id="htm">
        ...
        <div class="all">
            <div class="col-lg-12  borderBottom">
                <span class="curd font16">项目分类：</span>
                <a href="${prc}/common/project/list/${typeId}?gradeId=${gradeId }"  class="${empty classId ?'btn btn-list current_a':'btn btn-list'}">全部</a>
                <c:forEach items="${typeClass }" var="b" varStatus="i">
                     <a href="${prc}/common/project/list/${typeId}?classId=${b.id}&gradeId=${gradeId }"  class="${b.id eq classId ?'btn btn-list current_a':'btn btn-list'}"  >${b.name }</a>
                 </c:forEach>
            </div>
            <div class="col-lg-12">
                <span class="curd font16">适合年级：</span>
                <a href="${prc}/common/project/list/${typeId}?classId=${classId}"  class="${empty gradeId?'btn btn-list current_a':'btn btn-list'}" >全部</a>
                <c:forEach items="${grade}" var="g" varStatus="i">
                     <a href="${prc}/common/project/list/${typeId}?classId=${classId }&gradeId=${g.code }"   class="${g.code eq gradeId ?'btn btn-list current_a':'btn btn-list'}" >${g.cname }</a>
                 </c:forEach>
            </div>
        </div>
     ...
     <c:forEach items="${list }" var="p" varStatus="i">
        <div class="case-box">
            <!-- 左侧图片 -->
            <div class="caseLeft">
                <a href="${prc}/common/project/details/${p.id}"><img src="${prc}/image/${p.imgUrl}/cut002" alt=""> </a>
            </div>
            <!-- 右侧文字描述 -->
            <div class="caseRight  textCenter">
                <a class="font24" href="${prc}/common/project/details/${p.id}" >${p.name}</a>
                ...
                <ul class="more03 clear">
                  <c:forEach items="${p.cases}" var="b" varStatus="i"  begin="0" end="3">
                    <li class="position01">
                        <a  href="${prc}/common/case/detail?id=${b.id}"  >
...
<script>
	...
	function load(){
		$.ajax({
		    type: 'post',
		    url:'${prc}/common/project/page.ajax',
		    data:{page:page,limit:limit,typeId:typeId,classId:classId,gradeId:gradeId,name:name},
		    dataType: 'json',
		    			...
		    			var htm="";
		    				for (var i = 0; i < data.length; i++) {
		    					...
		    					var phtm='<div class="case-box">'
		    						    +'    <div class="caseLeft">'
		    						    +'       <a href="${prc}/common/project/details/'+data[i].id+'"><img src="${prc}/image/'+data[i].imgUrl+'/cut002" alt=""></a>'
		    						    +'    </div>'
		    						    +'    <div class="caseRight  textCenter">'
		    						    +'       <a class="font24" href="${prc}/common/project/details/'+data[i].id+'">'+data[i].name+'</a>'
		    						    +'       <p class="projectRight01">'
