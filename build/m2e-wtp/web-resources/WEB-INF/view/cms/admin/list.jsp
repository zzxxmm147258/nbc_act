        ...
        <div class="row wrapper white-bg page-heading">
            ...
            <div class="col-lg-3 fr" style="margin-top:33px;">
                <a href="#" style="margin-right:12px" data-toggle="modal" data-target="#addUser01">
                      <button class="btn btn-md btn-primary  current_a fr" style="margin-right:50px">添加管理员</button>
                </a>
            </div>
        </div>
    ...
    <div class="modal inmodal" id="addUser01" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <script>
            ...
            function saveAdmin(){
				...
				$.getJSON('${prc}/main/admin/check/user_name/'+userName,function(data){
            		if(data.success){ // 帐号可以使用
            			$.post('${prc}/main/admin/save',
            					{"userName":userName,"trueName":trueName,"password":password},
            					function(data){
            						if(data.success){
            							alert(data.message);
            							location.href="${prc}/main/admin/list";
            						}else{
            							alert(data.message);
            						}
            					});
            ...
            var options = {
           	    ...
           	    pageUrl:function(type,page, current){
           	    	return "${prc}/main/admin/list?page="+page
           	    }
           	}
            ...
            function remove(){
            	del_id = $('#del_id').val();
            	location.href='${prc}/main/admin/del/'+del_id
            }
          ...
          //删除按钮事件
        	$("a[name=remove]").click(function(){
        	  ...
        	  if(confirm("是否确定要删除？")){
        		...
        		$.ajax({
        			url:'<c:url value="/main/admin/del.ajax" />',
        			data:{id:id},
        			type:'post',
            ...
            $('input').blur(function(){
         		...
         		var password=$("#"+id+" input[name='password']" ).val();
         		 ...
         		 $.ajax({
        			url:'<c:url value="/main/admin/update.ajax" />',
        				...
        				if(data.success){
        					...
        					if(password){
        						$("#"+id+" input[name='password']" ).val(password);
          					    //$("#"+id+" input[name='password']" ).attr("placeholder",password);
          					}
        				}
        			},
        		 })
